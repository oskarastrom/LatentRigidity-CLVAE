...
def parse_args():
    ...
    parser.add_argument('--save_name', type=str, default='', help='Final save name inside "./checkpoints"')
    
    parser.add_argument('--learned_clusters', default=False, action=argparse.BooleanOptionalAction, help='If cluster should be learned. Default False.')
    parser.add_argument('--learned_variance', default=False,  action=argparse.BooleanOptionalAction, help='If cluster variances should also be learned. Default False.')
    parser.add_argument('--fixed_dim', type=int, default=0, help='Number of fixed dimensions if learned_clusters=True.')
    ...
    # Process args
    if args.dataset != "": args = process_args(args)
