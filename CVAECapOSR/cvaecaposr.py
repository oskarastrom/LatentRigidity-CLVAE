...
class CVAECapOSR(pl.LightningModule):
    def __init__(self, num_classes, in_channels, in_dim_caps=16, out_dim_caps=32, t_mu_shift=1.0, 
                 ...
                 fixed_dim=0, learned_variance=False):
        super(CVAECapOSR, self).__init__()
        self.fixed_dim = fixed_dim
        self.learn_variance = learned_variance
        self.test_outputs = []
        self.z_dim = z_dim
        ...
        self.z_dim_fixed = self.fixed_dim
        self.z_dim_learned = self.z_dim - self.fixed_dim
        self.t_mean_fixed = nn.Embedding(self.num_classes, self.num_classes * self.z_dim_fixed)
        self.t_mean_learned = nn.Embedding(self.num_classes, self.num_classes * self.z_dim_learned)
        self.t_var = nn.Embedding(self.num_classes, self.num_classes * z_dim)
        self.t_mean_fixed, self.t_mean_learned, self.t_var = self._init_targets(t_mu_shift, t_var_scale)

        #self.t_mean_fixed = scatter_on_hypersphere(self.num_classes, self.num_classes * self.fixed_dim)
        #t_mean_fixed *= t_mu_shift
            #self.t_mean_fixed = nn.Embedding.from_pretrained(t_mean_fixed.to(device), freeze=True)
    ...
    def _init_targets(self, t_mu_shift, t_var_scale):
        device = self.t_mean_learned.weight.device

        t_mean_init = 1.0 * F.one_hot(torch.arange(self.num_classes), self.num_classes)
        t_mean_init = t_mean_init.unsqueeze(-1).repeat(1, 1, self.z_dim_fixed).view(self.num_classes, -1)
        t_mean_init = t_mean_init * t_mu_shift
        t_mean_fixed = nn.Embedding.from_pretrained(t_mean_init.to(device), freeze=True)

        t_mean_init = 1.0 * F.one_hot(torch.arange(self.num_classes), self.num_classes)
        t_mean_init = t_mean_init.unsqueeze(-1).repeat(1, 1, self.z_dim_learned).view(self.num_classes, -1)
        t_mean_init = t_mean_init * t_mu_shift
        t_mean_learned = nn.Embedding.from_pretrained(t_mean_init.to(device), freeze=False)

        t_var_init = torch.ones(self.num_classes, self.num_classes * self.z_dim)
        t_var_init = t_var_init * t_var_scale
        t_var = nn.Embedding.from_pretrained(t_var_init.to(device), freeze=not self.learn_variance)

        return t_mean_fixed, t_mean_learned, t_var
    ...
    def cross_kl_div(self, z_mu, z_var, detach_inputs=False, detach_targets=False):
        ...
        if self.z_dim_fixed == 0: t_means = self.t_mean_learned(t_idxs)
        elif self.z_dim_learned == 0: t_means = self.t_mean_fixed(t_idxs)
        else: t_means = torch.cat([self.t_mean_learned(t_idxs), self.t_mean_fixed(t_idxs)], -1) # [bs, num_classes, num_classes * z_dim]
    ...
    def training_step(self, batch, idx_batch):
        ...
        if self.z_dim_fixed == 0: t_mu = self.t_mean_learned(y)
        elif self.z_dim_learned == 0: t_mu = self.t_mean_fixed(y)
        else: t_mu = torch.cat([self.t_mean_learned(y), self.t_mean_fixed(y)], -1)
        t_var = self.t_var(y)
        t_mu = t_mu.view(preds['z_mu'].shape)
        t_var = t_var.view(preds['z_var'].shape)
        
        # Loss
        loss_kl = kl_div(preds['z_mu'], preds['z_var'], t_mu, t_var).mean(-1).mean(0)
        ...
        # Cache
        self.log('train_loss_kl', loss_kl, batch_size=x.shape[0])
        self.log('train_loss_contr', loss_contr, batch_size=x.shape[0])
        self.log('train_loss_rec', loss_rec, batch_size=x.shape[0])
        self.log('train_loss', loss, batch_size=x.shape[0])
        self.log('train_acc', acc, prog_bar=True, batch_size=x.shape[0])
        return loss
    ...
    def validation_step(self, batch, idx_batch):
        ...
        # Cache
        self.log('validation_loss_kl', loss_kl, batch_size=x.shape[0])
        self.log('validation_loss_contr', loss_contr, batch_size=x.shape[0])
        self.log('validation_loss_rec', loss_rec, batch_size=x.shape[0])
        self.log('validation_loss', loss, batch_size=x.shape[0])
        self.log('validation_acc', acc, prog_bar=True, batch_size=x.shape[0])
    ...
    def test_step(self, batch, idx_batch):
        ...
        self.test_outputs.append(torch.stack([probs, y_ku], -1))

    def on_validation_epoch_end(self):
        #print("")
        #if np.prod(self.t_mean_learned.weight.shape) > 0: print("learned", self.t_mean_learned.weight[0,0].item(), self.t_mean_learned.weight.shape)
        #if np.prod(self.t_mean_fixed.weight.shape) > 0: print("fixed", self.t_mean_fixed.weight[0,0].item(), self.t_mean_fixed.weight.shape)
        #if np.prod(self.t_var.weight.shape) > 0: print("var", self.t_var.weight[0,0].item(), self.t_var.weight.shape)
        return super().on_validation_epoch_end()
    
    def on_test_epoch_end(self):
        auroc = AUROC(task="binary")#, pos_label=0?
        probs, y_ku = torch.cat(self.test_outputs, 0).T
        y_ku = 1-y_ku.to(torch.int64)
        self.log('test_auroc', auroc(probs, y_ku), prog_bar=True)
        self.test_outputs = []  # Clear the stored outputs for the next test run
...
def get_model(args, data_info):
    ...
    if len(args.checkpoint) > 0:
        model = CVAECapOSR.load_from_checkpoint(args.checkpoint, **model_args)
